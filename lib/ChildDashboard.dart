  ...
  void _calculateUpcomingVaccines() {
    ...
    if (nextSectionMonth == null) {
      ...
      vaccineScheduleMonth.forEach((key, month) {
        ...
        if (_isExpired(key, ageMonths)) return;
        final scheduledDate = DateTime(
            _birthDate!.year, _birthDate!.month + month, _birthDate!.day);
        if (sectionEarliest == null ||
            scheduledDate.isBefore(sectionEarliest!)) {
          sectionEarliest = scheduledDate;
        }
      });
      _nextAppointment = sectionEarliest ??
          DateTime(_birthDate!.year, _birthDate!.month + nextSectionMonth,
              _birthDate!.day);
    }
...
class _DashboardCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        padding:
            EdgeInsets.symmetric(horizontal: padding, vertical: padding + 6),
        decoration: BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.circular(8),
            border: Border.all(color: Colors.grey.shade300),
