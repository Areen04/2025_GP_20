...
class _ChildDashboardState extends State<ChildDashboard> {
  ...
  DateTime _effectiveUpcomingDate(DateTime scheduledDate, DateTime now) {
    final today = DateTime(now.year, now.month, now.day);
    return scheduledDate.isBefore(today) ? today : scheduledDate;
  }
  ...
  void _calculateUpcomingVaccines() {
    ...
    vaccineScheduleMonth.forEach((key, months) {
      if (!_isTaken(key)) {
        ...
        final effectiveDate = _effectiveUpcomingDate(scheduledDate, now);
    ...
    if (nextSectionMonth == null) {
      ...
      vaccineScheduleMonth.forEach((key, month) {
        ...
        final effectiveDate = _effectiveUpcomingDate(scheduledDate, now);
        if (sectionEarliest == null || effectiveDate.isBefore(sectionEarliest!)) {
          sectionEarliest = effectiveDate;
        }
      });
      _nextAppointment = sectionEarliest ??
          _effectiveUpcomingDate(
            DateTime(
              _birthDate!.year,
              _birthDate!.month + nextSectionMonth,
              _birthDate!.day,
            ),
            now,
          );
